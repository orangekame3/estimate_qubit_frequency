...
class QubitResponseConfig:
    ...
    def _validate(self):
        ...
        if self.f01_height_min < 1:
            raise ValueError('f01_height_min must be >= 1')
        ...
        if self.f12_height_min < 1:
            raise ValueError('f12_height_min must be >= 1')
...
class QubitResponse:
    ...
    def f01(self):
        ...
        if max_height < self.config.f01_height_min:
            return None
        ...
        moment = self.compute_moment(self.zs, self.zs_labeled, label)
    ...
    def f12(self):
        ...
        peaks = [
            ...
            and peak.height >= self.config.f12_height_min
        ]
    ...
    def peaks(self):
        ...
        heights = itertools.chain([0], self.heights, [0])
        for x, (height_prev, height) in enumerate(itertools.pairwise(heights)):
            if height > height_prev:
                x_start = x

            if height < height_prev and x_start is not None:
                _peaks.append(QubitResponse.Peak(x_start, x, height_prev))
                x_start = None
    ...
    @staticmethod
    def compute_moment(
        zs: npt.NDArray[np.float64], zs_labeled: npt.NDArray[np.int32], label: int
    ) -> float:
        ys, xs = np.where(zs_labeled == label)
        weights = np.abs(zs[ys, xs])
        lever = zs.shape[0] - ys - 0.5
        return float(np.sum(weights * lever))
...
def process_data(data, conf, image_dir_base=None, plot=False, json_output=False):
    ...
    if image_dir_base or plot:
        ...
        if image_dir_base:
            ...
            fig.write_image(
                os.path.join(image_dir, f'qubit_{qubit_idx}_{moment:04}.png')
            )
